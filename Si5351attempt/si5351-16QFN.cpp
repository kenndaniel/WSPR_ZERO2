/*
  * si5351-16QFN.h - Si5351 16QFN library for Arduino
 * 
...
#include "Arduino.h"
#include "Wire.h"
#include "si5351-16QFN.h"

int clock_ctl[] = {0,1,4,6};   // offset from SI5351_CLK0_CTRL for other clocks
/********************/
/* Public functions */
/********************/
...
uint8_t Si5351::set_freq(uint64_t freq, enum si5351_clock clk)
{
	...
	if((uint8_t)clk <= (uint8_t)SI5351_CLK2)
	{
...
void Si5351::set_ms(enum si5351_clock clk, struct Si5351RegSet ms_reg, uint8_t int_mode, uint8_t r_div, uint8_t div_by_4)
{
	...
	switch(clk)
	{
		...
		case SI5351_CLK2:
			...
			break;
		case SI5351_CLK3:
			si5351_write(SI5351_CLK3_PARAMETERS, temp);
			ms_div(clk, r_div, div_by_4);
			break;
		// case SI5351_CLK4:
...
void Si5351::output_enable(enum si5351_clock clk, uint8_t enable)
{
  ...
  if(enable == 1)
  {
    reg_val &= ~(1<<(uint8_t)clock_enable[clk]);
  }
  else
  {
    reg_val |= (1<<(uint8_t)clock_enable[clk]);
...
void Si5351::drive_strength(enum si5351_clock clk, enum si5351_drive drive)
{
  ...
  reg_val = si5351_read(SI5351_CLK0_CTRL + (uint8_t)clock_ctl[clk]);
  reg_val &= ~(mask);
  ...
  si5351_write(SI5351_CLK0_CTRL + (uint8_t)clock_ctl[clk], reg_val);
}
...
void Si5351::set_ms_source(enum si5351_clock clk, enum si5351_pll pll)
{
	...
	reg_val = si5351_read(SI5351_CLK0_CTRL + (uint8_t)clock_ctl[clk]);  // Change to case statement
 ...
 *
 * Set the state of the clock output when it is disabled. Per 
 * of AN1234 (Registers 17, 18, 21 and 23), there are four possible values: low,
...
void Si5351::set_clock_disable(enum si5351_clock clk, enum si5351_clock_disable dis_state)
{
	...
	if (clk >= SI5351_CLK0 && clk <= SI5351_CLK1)
	{
		reg = SI5351_CLK1_0_DISABLE_STATE;
	}
	else if(clk >= SI5351_CLK2 && clk <= SI5351_CLK3)
	{
		reg = SI5351_CLK3_2_DISABLE_STATE;
	}
	else return;
...
void Si5351::set_clock_fanout(enum si5351_clock_fanout fanout, uint8_t enable)
{
	...
	case SI5351_FANOUT_XO:
		...
		break;
	// case SI5351_FANOUT_MS:  Not avaialble in qfn16
	// 	if(enable)
	// 	{
