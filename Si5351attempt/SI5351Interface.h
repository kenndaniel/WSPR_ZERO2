...
#define XMIT_CLOCK0  SI5351_CLK3
#define XMIT_CLOCK1  SI5351_CLK5
...
{ 
  ...
  if(twoChanel) si5351.output_enable(XMIT_CLOCK1, 1);
  ...
  for (i = 0; i < symbol_count; i++) // Now transmit the channel symbols
  {
    ...
    if(twoChanel) si5351.set_freq((freq * 100) + (tx_buffer[i] * tone_spacing), XMIT_CLOCK1);
    if(twoChanel) si5351.set_clock_invert(XMIT_CLOCK1,one);
  ...
  if(twoChanel) si5351.output_enable(XMIT_CLOCK1, 0);   
